...
\item An optional string referring to the \parameter{mother_volume}, which defines the passive material the detector will be placed in.
  ...
  Note: if the \parameter{mother_volume} is a hollow material, only the non-hollow part of the material is considered part of the material. Placing a detector in the hollow part requires a different \parameter{mother_volume}.
\end{itemize}
...
The passive material configuration consists of a set of sections describing the passive materials in the geometry setup, with a syntax similar to the detector configuration.
...
\begin{itemize}
  ...
  This means the detector is rotated first around the world's X-axis, then around the world's Y-axis and then around the world's Z-axis.
  Alternatively the orientation can be set as Z-Y-X or X-Z-X extrinsic Euler angles, refer to Section~\ref{sec:models_geometry} for details.
  \item An optional string referring to the \parameter{mother_volume}, which defines the volume where the passive material will be placed in.
  ...
  Note: if the \parameter{mother_volume} is a hollow material, only the non-hollow part of the material is considered part of the material. Placing a volume in the hollow part requires a different \parameter{mother_volume}.
  ...
  \item A set of \parameter{size} parameters specific for the model that is chosen. 
...
\textbf{Box}: A rectangular box which can be massive or have an hole in the middle along the z-axis.
\begin{itemize}
\item The \parameter{size} of the box is an XYZ vector which defines the total size of the box.
\item (Optional) The \parameter{inner_size} of the box is an XY vector which defines the size of the hole which will be removed from the original box volume. Defaults to 0mm 0mm.
\item (Optional) The \parameter{thickness} of the box is a value which defines the thickness of the walls of a box with a hole in the middle. This has the has the same effect as the \parameter{inner_size}, and such they can't be used together.
 Defaults to 0mm.
\end{itemize}

\textbf{Cylinder}: A cylindrical tube which can be massive or have an hole in the middle along the z-axis.
\begin{itemize}
...
Note that the if the \parameter{arc_length} is set to 360 degrees, the \apsq framework will always draw the full circumference, regardless of the value of \parameter{starting_angle}.
\end{itemize}

\textbf{Sphere}: A full or partly made sphere with an inner- and outer radius.
\begin{itemize}
...
\item (Optional) The \parameter{starting_angle_theta} of the sphere is the polar angle at which the \parameter{arc_length_theta} will start. 0 degrees refers to the point along the positive z-axis. Defaults to 0deg.
\item (Optional) The \parameter{arc_length_theta} of the sphere is the arc-length of the polar angle which will be rotated around the z-axis to build the sphere, starting from the given \parameter{starting_angle_theta}. Defaults to 100deg which creates the full circle.\\
Note that \parameter{arc_length_phi} works the same as the \parameter{arc_length} from the cylinder, but the \parameter{arc_length_theta} works different.
...
Note: If the VisualizationGeant4 module is used in conjunction with and \parameter{arc_length_theta} different from 180deg, the Visualization GUI will show an error "Inconsistency in bounding boxes for solid". The origin of this error is unknown but the error can be ignored.
\end{itemize}
...
\item \parameter{passive_materials_file}: Location of the file describing the passive materials configuration (introduced in Section~\ref{sec:passive_material_config}).
