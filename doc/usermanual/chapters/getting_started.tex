...
\item \textbf{\texttt{purge\_output\_directory}}: Decides whether the content of an already existing output directory is deleted before a new run starts. Defaults to \texttt{false}, i.e. files are kept but will be overwritten by new files created by the framework.
\item \textbf{\texttt{deny\_overwrite}}: Forces the framework to abort the run and throw an exception when attempting to overwrite an existing file. Defaults to \texttt{false}, i.e. files are overwritten when requested. This setting is inherited by all modules, but can be overwritten in the configuration section of each of the modules.
...
\item \textbf{\texttt{random\_seed\_core}}: Optional seed used for pseudo-random number generators in the core components of the framework. If not set explictely, the value $(\mathrm{\texttt{random\_seed}} + 1)$ is used.
...
\item \textbf{\texttt{experimental\_multithreading}}: Enables EXPERIMENTAL multithreading for the framework. This can speed up simulations of multiple detectors significantly. More information about the multithreading in Section \ref{sec:multithreading}.
