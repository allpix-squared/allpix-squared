...
Each section starts with a header describing the name used to identify the detector; all names are required to be unique and thus using the same detector name multiple times is forbidden.
...
\item \parameter{experimental_multithreading}: Enable EXPERIMENTAL multithreading for the framework. This can speed up simulations of multiple detectors significantly. More information about multithreading can be found in Section \ref{sec:multithreading}.
...
\item The \textbf{deposition} module that simulates the particle beam that creates charge carriers in the detectors using the provided physics list (containing a description of the simulated interactions) and the geometry created above.
...
Only the charge carriers deposited in the sensors of the Timepix detector models will be propagated and digitized.
Finally, some detector histograms for the device under test (DUT) will be recorded as ROOT histograms and all simulated objects (including the Monte Carlo truth) will be stored in the \apsq ROOT file.
...
If modules or models fail to load, more information about potential issues with the library loading can be found in the detailed framework description in Section~\ref{sec:framework}.

\subsection{Adding More Modules}
A few basic modules are now discussed, which may be of use during a first simulation.
...
The example configuration above contained a module which added a linear electric field to the detectors.
...
The \texttt{ROOTObjectWriter} automatically saves all objects created to a TTree~\cite{roottree}.
