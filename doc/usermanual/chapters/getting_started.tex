...
For simulating the detector response in the presence of a magnetic field with \apsq, a constant magnetic field can be defined. By default, it is turned off. A field can be added using the \parameter{MagneticFieldReader} module, giving the field vector as parameter:
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos]{ini}
...
The magnetic field is used by the interface to Geant4 and therefore exposes charged primary particles to the Lorentz force, and as a property of each detector present, enabling a Lorentz drift of the charge carriers in the active sensors. By now, in contrast to the \parameter{GenericPropagation} module, the \parameter{ProjectionPropagation} module does not feature a Lorentz drift. However, in both modules, the effect of the magnetic field can be ignored intentionally by enabling the parameter \parameter{ignore_magnetic_field} for the corresponding propagator module.
