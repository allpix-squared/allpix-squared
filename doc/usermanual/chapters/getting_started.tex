...
\item \textbf{\texttt{experimental\_multithreading}}: Enable EXPERIMENTAL multithreading for the framework. This can speed up simulations of multiple detectors. Currently multithreading is still in beta, use at your own risk!
\item \textbf{\texttt{workers}}: Specify the amount of workers to use in total, should be strictly larger than zero. Only used if \texttt{experimental\_multithreading} is set to true. Defaults to the number of native threads available on your system if this can be determined. 
\end{itemize}
