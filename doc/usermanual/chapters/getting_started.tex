...
\begin{itemize}
\item A \textbf{model reader}, which reads the model paths and creates the detector models it can parse (should all be models specified in the detector configuration). In the current version this means almost every simulation should contain the \texttt{DefaultModelReader} module at the top of the main configuration.
