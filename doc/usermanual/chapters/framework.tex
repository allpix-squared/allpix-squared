...
The framework allows to parallelize the execution of the same module for multiple events if the module supports it.
If not, the module is run for a single event as a time.
...
\end{minted}
By adding this, the module promises that it will work correctly if the run-method is executed multiple times in parallel, in separate event instantiations.
...
It is recommended that a module does not access/modify any member variables in its \parameter{run()}-method for increased performance.
