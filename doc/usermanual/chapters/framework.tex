...
The available modules and their parameters are discussed in detail in Section~\ref{ch:modules}.
...
More information about the messaging system and the supported objects can be found in Section~\ref{ch:objects_messages}.
...
More information about the tools can be found in Section~\ref{ch:additional_tools_resources}.
...
The different subcomponents such as configuration, modules and messages are discussed in Sections~\ref{sec:config_parameters} to ~\ref{ch:objects_messages}.
...
Messages with objects are also dispatched through the messenger as described in Section~\ref{ch:objects_messages}.
...
It is automatically converted to \LaTeX using Pandoc~\cite{pandoc} and included in the user manual in Section~\ref{ch:modules}.
...
The pointer to a Messenger can be used to bind variables to either receive or dispatch messages as explained in~\ref{ch:objects_messages}.
...
This feature is disabled for new modules by default, and has to be both supported by the module and enabled by the user as described in Section \ref{ch:framework_parameters}.
...
Section~\ref{ch:modules} provides details about the materials supported by the geometry builder module (\parameter{GeometryBuilderGeant4}).
...
The list of objects available in the \apsq objects library are discussed in Section~\ref{ch:objects}.
