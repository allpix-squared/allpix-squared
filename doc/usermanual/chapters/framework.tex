...
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos]{c++}
// Enable parallelization of this module if multi-threading is enabled
...
Detailed description of how the framework implements the multi-threading feature can be found in Section~\ref{sec:multithreading_approach}.
...
This thread pool has a fixed queue size so that not all events are submitted at the same time and consume additional memory.

By default, \apsq assumes the module doesn't support parallelization and each module must explicitly enable multi-threading in its constructor as described in Section~\ref{sec:multithreading}.
To support multi-threading, the module's \texttt{run()} method should be re-entrant and any shared member variables should be protected.
If multi-threading is enabled, the module manager will check if all the loaded modules support multi-threading.
...
A stable seed distribution to modules and core components of \apsq is guaranteed in order to be able to exactly reproduce a simulation result from the same inputs even when the number of workers is different.
Each event holds a random engine that can be used by a module in it's \texttt{run()} method.
\apsq has a master random engine that is used to seed such engines for every event in a sequential manner, in order of increasing event number.

To avoid the memory overhead of maintaining random engine objects equal to the number of events, the storage of such objects is thus made static thread local.
...
\subsection{Running Events in order using BufferedModule}
The \texttt{BufferedModule} class is made available for modules that are interested in running events in the correct order without disabling multi-threading.
Inheriting from this class will allow the module to transparently check if the given event is in order and decide whether to execute it immediately or to store it in a buffer if it is out of order.
It should be noted that such modules cannot dispatch messages to the framework.

Using the \texttt{BufferedModule} is more suitable for the writer modules that runs at the end of each event.
It allows the writers to produce the exact same output file for the same simulation inputs without sacrificing the benefits of using multi-threading for other modules.

\subsection{Geant4 Modules}
The usage of the Geant4 library in \apsq has some constraints because the Geant4 multi-threaded run manager expects to handle parallelization internally which violates \apsq design.
Furthermore, Geant4 does not guarantee results reproducibility between both its multi-threaded and sequential run managers.
Modules that would like to use the Geant4 library shall not use the run managers provided by Geant4.
Instead, they must use the custom run managers provided by \apsq as described in Section~\ref{sec:geant4_interface}.

\subsection{Issues with ROOT Dependency}
ROOT \texttt{TObject} and \text{TRef} classes are used by all objects in \apsq to relate to other events.
Because of legacy reasons, they do not scale well to a high number of threads.
