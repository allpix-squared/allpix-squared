...
\end{minted}
\item Add a dependency to keep messages in memory for fetching history.
Several objects have related objects which can be retrieved as explained in Section \ref{sec:objhistory}.
Those related objects can however only be retrieved (during the same event) if these source objects remain in memory.
Modules should explicitly mark objects that are fetched indirectly through history, to ensure these objects are not deleted.
