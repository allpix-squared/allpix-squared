...
It creates a thread pool with the configured number of workers or determines them from system parameters if not specified.
...
A stable seed distribution to modules and core components of \apsq is guaranteed in order to be able to exactly reproduce a simulation result from the same inputs.
...
To avoid the memory overhead of maintaining random engine objects equal to the number of events, the storage of such object is thus made static thread local.
...
Therefore, the \texttt{LocalMessenger} was created to handle communication local to an event.
Each event has its own copy of the local messenger, however, all communication is managed by the main messenger class for ease of use.

The main messenger class handles subscription to specific messages by modules.
...
When working in a multi-threaded context, Geant4 expects to handle the parallel work internally; this clashes with the design of \apsq.
