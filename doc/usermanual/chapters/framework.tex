...
The framework currently only allows to parallelize the execution phase of the same type of module if these would otherwise be executed directly after each other in the linear order. Thus as long as the name of the module remains the same, while going through the execution order of all the \texttt{run()} methods, all instances are added to a work queue. The instances are then distributed to a set of worker threads as specified in the configuration which will execute the individual modules. The module manager will wait for all the jobs to finish, before it will continue to handle the next type of module.
...
\end{minted}
By adding this, the module promises that it will work correctly if the run-method is executed multiple times in parallel in separate instantiations. This means in particular that the module will safely handle access to shared (for example static) variables and it will properly bind ROOT histograms to their directory before the \texttt{run()}-method. Access to constant operations in the GeometryManager, Detector and DetectorModel is safe between various threads. Also sending and receiving messages is guarenteed to be thread-safe.
...
Section~\ref{sec:modules} provides details about the materials currently supported by the geometry builder module (\texttt{GeometryBuilderGeant4}).
...
Can be used for more advanced strategies of retrieving messages.
Note that this method can lead to unexpected behavior because the listener function is executed during the run method of the dispatching module.
