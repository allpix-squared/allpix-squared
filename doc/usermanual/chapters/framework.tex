...
Within the \texttt{finalize()} method, every module has access to the full final framework configuration via the \texttt{get\_final\_configuration()} method.
...
\end{itemize}
\item The \textbf{orientation} to specify the Euler angles in logical order (first x, then y, then z), interpreted using the method above (or with the \texttt{xyz} method if the \textbf{orientation\_mode} is not specified). An example for three Euler angles would be
...
If \textbf{orientation\_mode} is set to \texttt{matrix}, a $3\times3$ rotation matrix is expected. The matrix uses the row-major C convention, i.e.\ different rows of the rotation matrix are interpreted as row vectors and the configuration
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos]{ini}
