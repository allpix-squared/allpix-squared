...
This simpilfies calculations in the local coordinate system as all positions can either be stated in absolute numbers or in fractions of the pixel pitch.
...
In order to ensure receiving a specific detector model from the base class, the model should be downcasted.
...
As usual, the message is dispathed at the end of the \texttt{run} function of the module.
\begin{minted}[frame=single,framesep=3pt,breaklines=true,tabsize=2,linenos]{c++}
void run(unsigned int event_id) {
...
Note that this method can lead to unexpected behaviour because the listener function is executed during the run method of the dispatching module.
