...
Each event holds a random engine that can be used by a module in its \texttt{run()} method.
...
To avoid the memory overhead of maintaining random engine objects equal to the number of events, the storage of such objects is thus made static and thread-local, and is only provided to the event for temporary usage.
...
The \texttt{BufferedModule} class is made available for modules that require processing of events in the correct order without disabling multi-threading.
...
Using the \texttt{BufferedModule} is suitable for I/O modules which read or write to the file system.
It e.g.\ allows output modules to produce the exact same output file for the same simulation inputs without sacrificing the benefits of using multi-threading for other modules.
