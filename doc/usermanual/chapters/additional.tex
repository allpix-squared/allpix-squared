...
The GenricPropagation module uses Runge-Kutta integrator with the Runge-Kutta-Fehlberg method (RKF5 tableau).
After the integrator has been instantiated with its initial position, its \command{step()} function then allows to advance to the next step.
...
The \command{getValue()} and \command{setValue()} methods allow to retrieve, alter and update the position.
...
The constructor of the parser takes the units in which the field data should be interpreted, it is automatically converted to the framework base units described in Section~\ref{sec:config_values}. The file path provided to the field parser should always be canonical, if the file is not found or cannot be parsed, a \command{std::runtime_error} exception is thrown.
