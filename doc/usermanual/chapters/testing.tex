...
It derives from a CentOS\,7 Docker image and can be build using the \file{etc/docker/Dockerfile.base} file via the following commands:
...
\begin{description}
    ...
    \item[\file{test_1-2_geobuilder_addpoint.conf}] ensures the module adds corner points of the passive material in a correct way.
    \item[\file{test_1-3_geobuilder_addpoint_rot.conf}] ensures proper rotation of the position of the corner points of the passive material.
    \item[\file{test_1-4_geobuilder_mothervolume.conf}] ensures placing a detector inside a passive material will not cause overlapping materials.
    \item[\file{test_1-5_geobuilder_worldvolume.conf}] ensures the added corner points of the passive material increase the world volume accordingly.
    \item[\file{test_1-6_geobuilder_same_materials.conf}] tests if a warning will be thrown if the material of the passive material is the same as the material of the world volume.
    ...
    \item[\file{test_02-5_magneticfield_local.conf}] checks that the local magnetic field including the detector rotation is correct.
    \item[\file{test_02-6_electricfield_linear_depletion_side.conf}] checks that depleting from the sensor backside is possible.
    ...
    \item[\file{test_05-1_transfer_simple.conf}] tests the transfer of charges from sensor implants to readout chip. The monitored output comprises the total number of charges transferred and the coordinates of the pixels the charges have been assigned to.
    \item[\file{test_05-2_transfer_simple_implant.conf}] tests the transfer of charges from sensor implants to readout chip in case sensor implants have been defined in the detector model.
    \item[\file{test_05-3_transfer_pseudopulse.conf}] tests the calculation of induced signals based on the arrival time of charge carriers at the sensor surface.
    ...
    \item[\file{test_06-6_digitization_csa_pseudopulse.conf}] checks the outcome of a digitization by the CSA of a pseudo-pulse generated from arrival times.
    \item[\file{test_06-7_digitization_csa_tot_pseudopulse.conf}] checks the outcome of a digitization by the CSA of a pseudo-pulse generated from arrival times and conversion to time-over-threshold units.
