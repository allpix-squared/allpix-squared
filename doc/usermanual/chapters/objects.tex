...
Every MCParticle carries a unique track ID to associate it to a trajectory. MCParticles which have the same track ID belong to the same trajectory.
The MCParticle also stores an identifier of the particle type, using the PDG particle codes~\cite{pdg}.
The MCParticle additionally stores a parent MCParticle object, if available. The lack of a parent doesn't guarantee that this MCParticle originates from a primary particle, but only means that no parent on the given plane exists.
...
Regarding trajectory history, the raw Geant4 trajectories are not stored and thus not available. However, every MCParticle can be linked to a track with the associated track ID. Track hirarchy can be derived using the entire set of MCParticles. If a MCParticle is linked via the parent-child relationship on one plane, the tracks can be globally linked this way sa well.
