...
Regarding trajectory history, the raw Geant4 trajectories are not stored and thus not available.
However, every MCParticle can be linked to a track with the associated track ID.
Track hirarchy can be derived using the entire set of MCParticles.
If a MCParticle is linked via the parent-child relationship on one plane, the tracks can be globally linked this way as well.
