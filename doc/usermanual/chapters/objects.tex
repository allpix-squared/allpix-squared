...
Together with the MCParticles, the full inforamtion of the Monte-Carlo trajectory is stored.
Aside from this, the MCTrack stores information on the initial and final point of the trajectory (in \underline{global} coordinates), the energies (total as well as kinetic only) at those points, the creation process type, name, and the volume it took place in.
Furthermore, the particle id is stored via its PDG particle code~\cite{pdg}.
