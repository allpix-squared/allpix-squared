...
\item Boost.Random~\cite{boostrandom}: Random number generator and distribution library. Used in order to get cross-platform portable, STL-compatible random number distributions. While STL random number generators are portable and guarantee to deliver the same random number sequence given the same seed, random distributions do not and their implementation is platform-dependent leading to different results.
\end{itemize}
