...
\item[How can I run the exact same type of module with different settings?] This is possible by using the \parameter{input} and \parameter{output} parameters of a module that specialize the messages of the module.
...
Because getting the Monte Carlo truth of a PixelHit is quite a common thing a \parameter{getMCParticles} method is available which searches all layers of the history and returns an exception if any of the in between steps is not available or not loaded.
