...
namespace detail {
    ...
    public:
        call_combine_discontinuous(
            BidirIter first1, BidirIter last1, D d1, BidirIter first2, BidirIter last2, D d2, Function& f)
            : f_(f), first1_(first1), last1_(last1), d1_(d1), first2_(first2), last2_(last2), d2_(d2) {}

        bool operator()() { return combine_discontinuous(first1_, last1_, d1_, first2_, last2_, d2_, f_); }
    };
...
template <class Int> typename std::enable_if<!std::is_unsigned<Int>::value, void>::type check_non_negative(Int d1, Int d2) {
    if(d1 < Int(0) || d2 < Int(0))
        throw std::invalid_argument("expected non-negative argument");
}

template <class UInt> UInt count_each_combination(UInt d1, UInt d2) {
...
template <class BidirIter> std::uintmax_t count_each_combination(BidirIter first, BidirIter mid, BidirIter last) {
...
template <class UInt> UInt count_each_permutation(UInt d1, UInt d2) {
    // return (d1+d2)!/d2!
    check_non_negative(d1, d2);
    if(d1 > std::numeric_limits<UInt>::max() - d2)
...
template <class BidirIter> std::uintmax_t count_each_permutation(BidirIter first, BidirIter mid, BidirIter last) {
...
template <class UInt> UInt count_each_circular_permutation(UInt d1, UInt d2) {
...
template <class BidirIter> std::uintmax_t count_each_circular_permutation(BidirIter first, BidirIter mid, BidirIter last) {
...
namespace detail {
    ...
    public:
        rev3(Function f,
             ...
             BidirIter last3,
             D d3)
            : f_(f), first1_(first1), last1_(last1), d1_(d1), first2_(first2), last2_(last2), d2_(d2), first3_(first3),
              last3_(last3), d3_(d3) {}
...
template <class UInt> UInt count_each_reversible_permutation(UInt d1, UInt d2) {
...
template <class BidirIter> std::uintmax_t count_each_reversible_permutation(BidirIter first, BidirIter mid, BidirIter last) {
...
Function for_each_reversible_circular_permutation(BidirIter first, BidirIter mid, BidirIter last, Function f) {
    for_each_combination(
        first, mid, last, detail::reverse_circular_permutation<Function&, BidirIter>(f, std::distance(first, mid)));
    return std::move(f);
}

template <class UInt> UInt count_each_reversible_circular_permutation(UInt d1, UInt d2) {
