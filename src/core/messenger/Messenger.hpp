...
namespace allpix {
    ...
    public:
        ...
        template <typename T, typename R> void registerListener(T *receiver, void (T::*)(std::shared_ptr<R>), const std::string &message_type = "");

        // Register a single bind
        // FIXME: better names?
        template <typename T, typename R> void bindSingle(T *receiver, std::shared_ptr<R> T::*, const std::string &message_type = "");
        template <typename T, typename R> void bindMulti(T *receiver, std::vector<std::shared_ptr<R>> T::*, const std::string &message_type = "");
    ...
    template <typename T, typename R> void Messenger::registerListener(T *receiver, void (T::*method)(std::shared_ptr<R>), const std::string &message_type) {
    ...
    template <typename T, typename R> void Messenger::bindSingle(T *receiver, std::shared_ptr<R> T::*member, const std::string &message_type) {
        static_assert(std::is_base_of<Module, T>::value, "Receiver should have Module as a base class");
    ...
    template <typename T, typename R> void Messenger::bindMulti(T *receiver, std::vector<std::shared_ptr<R>> T::*member, const std::string &message_type) {
        static_assert(std::is_base_of<Module, T>::value, "Receiver should have Module as a base class");
        static_assert(std::is_base_of<Message, R>::value, "Bound variable should be a shared pointer to a message derived from the Message class");
