namespace allpix {
    ...
    template <typename T> void Messenger::dispatchMessage(Module *source, const T& msg, const std::string& name) {
    ...
    template <typename T> void Messenger::dispatchMessage(Module *source, std::shared_ptr<T> msg, const std::string& name) {
        static_assert(std::is_base_of<BaseMessage, T>::value, "Dispatched message should inherit from Message class");
        dispatch_message(source, std::static_pointer_cast<BaseMessage>(msg), name);
    }
