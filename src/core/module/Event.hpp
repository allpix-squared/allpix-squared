...
namespace allpix {
     ...
     */
    struct EventContext {
        EventContext(Messenger& messenger, ModuleList modules, std::map<Module*, long double>& module_execution_time, std::atomic<bool>& terminate)
            : messenger_(messenger), modules_(modules), module_execution_time_(module_execution_time), terminate_(terminate) {}
    ...
    public:
         ...
         */
        template <typename T> std::shared_ptr<T> fetchMessage() { return context_->messenger_.fetchMessage<T>(current_module_); }
