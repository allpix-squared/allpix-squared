...
std::vector<std::pair<ModuleIdentifier, Module*>> ModuleFactory::createModulesPerDetector(std::string moduleName, void* library){
    ...
    if(conf.has("name")) {
        ...
        for(auto& name : names) {
            ...
            Module* module = reinterpret_cast<Module* (*)(Configuration, Messenger*, std::shared_ptr<Detector>)>(generator)(conf, getMessenger(), det);
            moduleList.emplace_back(identifier, module);
