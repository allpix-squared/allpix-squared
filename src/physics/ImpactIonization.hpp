...
namespace allpix {
    ...
    public:
        ...
        double operator()(const CarrierType& type, double efield_mag, double threshold, double step) const override {
            ...
            if(type == CarrierType::ELECTRON) {
                return std::exp(step * electron_ac_ * efield_mag *
                                std::exp(-1 * electron_bd_ * electron_bd_ / efield_mag / efield_mag));
            } else {
                return std::exp(step * hole_ac_ * efield_mag * std::exp(-1 * hole_bd_ * hole_bd_ / efield_mag / efield_mag));
            }
        };
    ...
    public:
        ...
        double operator()(const CarrierType& type, double efield_mag, double threshold, double step) const override {
            ...
            if(type == CarrierType::ELECTRON) {
                return std::exp(step * efield_mag /
                                (electron_a_ + electron_b_ * std::exp(electron_d_ / (efield_mag + electron_c_))));
            } else {
                return std::exp(step * efield_mag / (hole_a_ + hole_b_ * std::exp(hole_d_ / (efield_mag + hole_c_))));
            }
        };
    ...
    public:
        ...
        ImpactIonization(const std::string& model, double temperature) {
            if(model == "massey") {
                ...
                model_ = std::make_unique<Bologan>(temperature);
            } else if(model == "none") {
