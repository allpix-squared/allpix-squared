...
DetectorConstruction::~DetectorConstruction(){
  vector<GeoDsc*>::iterator detItr = m_geoMap.begin();
...
void DetectorConstruction::run()
{
  ...
  if( m_userDefinedGeoOutputFile.Length() ){
...
void DetectorConstruction::Construct()
{
  ...
  */
  ...
  TGeoVolume *expHall_log = gGeoManager->MakeBox("ExpHall",m_fillingWorldMaterial,halfworld_dx,
					  halfworld_dy,halfworld_dz);
  ...
  for( ; detItr != m_geoMap.end() ; detItr++ )
    {
      ...
      cout << "Start detector " << dsc->GetID() << endl;
      ...
      if ( m_vectorWrapperEnhancement.find(id) != m_vectorWrapperEnhancement.end() )
	{
      ...
      cout << "Wrapper Dimensions [mm] : "
	   << TString::Format("hX=%3.3f hY=%3.3f hZ=%3.3f",wrapperHX,wrapperHY,
			      wrapperHZ)  << endl;
      
      // The wrapper logical volume
      TGeoVolume* wrapper_log = gGeoManager->MakeBox( WrapperName+id_s,
      ...
      TGeoCombiTrans* det_tr = new TGeoCombiTrans(posWrapper,m_rotVector[id]);
      ...
      TGeoVolume* Slice_log = Wafer_log->Divide(SliceName+id_s,1,
					      dsc->GetNPixelsX(),0,1,0,"N");
      //Slice_log->SetVisibility(false);
      // Replication along Y axis
      TGeoVolume *Pixel_log = Slice_log->Divide(PixelName+id_s,2,
						dsc->GetNPixelsY(),0,1,0,"N");
	...
	Wafer_id_1\Slice_id_[1,NPixelsX]\Pixel_id_[1,NPixelsY]
      */
      ...
      TGeoTranslation* posDevice = new TGeoTranslation("LocalDevTranslation"+id_s,
						       0.,0.,0.);
	...
	TGeoSphere* Bump_Sphere = new TGeoSphere( BumpSphereName,
						  0, // internal radius
						  dsc->GetBumpRadius() // ext radius
						  );
	TString BumpTubeName = BumpName+"Tube"+id_s;
	TGeoTube* Bump_Tube = new TGeoTube( BumpTubeName,
					    0., // internal radius
					    // external radius
	...
	TGeoCompositeShape* Bump =
	  new TGeoCompositeShape( BumpName+"Shape"+id_s,
				  BumpSphereName+"+"+BumpTubeName);
	...
	TGeoVolume* Bumps =
	  new TGeoVolume( BumpName+id_s, Bump, solder_med );
	...
	for( int ix=0; ix<dsc->GetNPixelsX(); ++ix ){
	  ...
	  for( int iy=0; iy<dsc->GetNPixelsY(); ++iy ){
	    ...
	    // Positions
	    double XPos = (ix*2+1)*dsc->GetHalfPixelX() - dsc->GetHalfSensorX() +
	      dsc->GetBumpOffsetX();
	    double YPos = (iy*2+1)*dsc->GetHalfPixelY() - dsc->GetHalfSensorY() +
	      dsc->GetBumpOffsetY();
	    TString xy_s = Form("_%i_%i",ix,iy);
	    TGeoTranslation* posBump =
	      new TGeoTranslation("LocalBumpTranslation"+id_s+xy_s, XPos, YPos, 0. );
	...
	TGeoVolume *Chip_log = gGeoManager->MakeBox(ChipName+id_s, Si_med,
						     dsc->GetHalfChipX(),
						     dsc->GetHalfChipY(),
						     dsc->GetHalfChipZ() );
	...
	// Placement !
	TGeoTranslation* posChip = new TGeoTranslation("LocalChipTranslation"+id_s,
						       dsc->GetChipXOffset(),
						       dsc->GetChipYOffset(),
						       dsc->GetChipZOffset()
						       - dsc->GetHalfSensorZ()
	...
	/*
	  ...
	  ### Change that policy ?
	 */
	TGeoMedium* Cover_med = gGeoManager->GetMedium( dsc->GetCoverlayerMat() );
	if( !Cover_med ){
	...
	TGeoTranslation* posCover =
	  new TGeoTranslation("LocalCoverlayerTranslation"+id_s,
			      0.,
			      0.,
			      -dsc->GetHalfSensorZ() - dsc->GetHalfCoverlayerZ() );
	...
	wrapper_log->AddNode(Cover_log,1,posCover);
      ...
      TGeoBBox* Box_GuardRings_Ext = new TGeoBBox( GuardRingsExtName,
						   dsc->GetHalfSensorX()
						   + dsc->GetSensorExcessHRight()
						   + dsc->GetSensorExcessHLeft(),
						   dsc->GetHalfSensorY()
						   + dsc->GetSensorExcessHTop()
						   + dsc->GetSensorExcessHBottom(),
						   // same depth as the sensor
						   dsc->GetHalfSensorZ() 
						   );
  
      TGeoCompositeShape* Solid_GuardRings =
	new TGeoCompositeShape(GuardRingsName+id_s,
			       //GuardRings = GuardRings_Ext - Wafer
			       GuardRingsExtName+"-"+Wafer_box->GetName() );

      // Create logical volume
      TGeoVolume* GuardRings_log = new TGeoVolume( GuardRingsName+id_s,
						   Solid_GuardRings, Si_med );
...
void DetectorConstruction::BuildAppliances(){
  ...
  TGeoTranslation* ApplTransl = NULL; // Type-depending Translation vector.
  ...
  for( ; aplItr != m_posVectorAppliances.end() ; aplItr++) {
    ...
    TGeoTranslation* ApplTranslItr =
      new TGeoTranslation("ApplianceTransl"+id_s, 0., 0., 0. );
...
void DetectorConstruction::BuildMaterialsAndMedia(){
  ...
  */
  ...
  TGeoMaterial* vacuum_mat = new TGeoMaterial("Vacuum",a,z,density);
  TGeoMedium* vacuum_med = new TGeoMedium("Vacuum",1,vacuum_mat);
...
Material:   G4_AIR    density:  1.205 mg/cm3  RadL: 303.921 m    Nucl.Int.Length: 710.095 m  
  ...
  */
  TGeoElement* N = new TGeoElement("Nitrogen", "N", z=7 , a=14.007);
  TGeoElement* O = new TGeoElement("Oxygen"  , "O", z=8 , a=15.999);
  TGeoElement* C = new TGeoElement("Carbon"  , "C", z=6 , a=12.011);
  TGeoElement* Ar = new TGeoElement("Argon"  , "Ar", z=18 , a=39.948);
  TGeoMixture* air_mat = new TGeoMixture("Air", 4, density=1.205E-3);
  air_mat->AddElement(N, 0.7844);
  air_mat->AddElement(O, 0.2107);
  air_mat->AddElement(C, 0.0002);
  air_mat->AddElement(Ar, 0.0047);
  TGeoMedium* air_med = new TGeoMedium("Air", ++numed, air_mat);
  ...
  */
  ...
  TGeoMaterial* Si_mat = new TGeoMaterial("Si",Si,density=2.330);
  TGeoMedium* Si_med = new TGeoMedium("Si",++numed,Si_mat);

  /* Epoxy
     AllPix1 uses G4_PLEXIGLASS
  */
  TGeoElement* H = table->FindElement("H");
  TGeoMixture* plexiglass_mat = new TGeoMixture("Plexiglass",3,density=1.19);
  plexiglass_mat->AddElement(C,5);
  plexiglass_mat->AddElement(H,8);
  plexiglass_mat->AddElement(O,2);
  TGeoMedium* plexiglass_med = new TGeoMedium("Plexiglass",++numed,plexiglass_mat);
  
  /* Solder SnPb */
  TGeoElement* Sn = new TGeoElement("Tin", "Sn", z=50 , a=118.710);
  TGeoElement* Pb = new TGeoElement("Lead","Pb", z=82., a=207.2);
  TGeoMixture *solder_mat = new TGeoMixture("Solder", 2, density=8.4);
  solder_mat->AddElement(Sn,63);
  solder_mat->AddElement(Pb,37);
  TGeoMedium* solder_med = new TGeoMedium("Solder",++numed,solder_mat);

  /* Aluminum
     AllPix1 uses G4_Al
  */
  TGeoElement* Al = table->FindElement("Al");
  TGeoMaterial* Al_mat = new TGeoMaterial("Al",Al,density=2.699);
  TGeoMedium* Al_med = new TGeoMedium("Al",++numed,Al_mat);
