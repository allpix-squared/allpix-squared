...
By default, the gain is integrated throughout the charge carrier propagation, and a new set of charge carriers is generated when an integer boundary is passed. An alternative approach can be selected via `multiplication_probability_based`, where the probability of impact ionization is calculated for every propagation step and every charge carrier in the group, and the decision on an impact ionization process is taken based on random numbers.
...
* `multiplication_probability_based`: Switches impact ionization algorithm to a probability based one, where the probability of impact ionization is calculated for every propagation step and every charge carrier in the group, and the decision on an impact ionization process is taken based on random numbers. Defaults to `false`, such that charge carriers are generated when the integrated gain passes an integer boundary.
* `multiplication_probability_samples`: Samples drawn for the probability based multiplication approach per propagation step per charge carrier. The probability is decreased accordingly. The default value is `20`.
