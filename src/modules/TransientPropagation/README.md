...
By default, the gain is integrated throughout the charge carrier propagation, and a new set of charge carriers is generated when an integer boundary is passed. An alternative approach can be selcted via `multiplication_probability_based`, where the probability of impact ionization is calculated for every propagation step and every charge carrier in the group, and the decision on an impact ionization process is taken based on random numbers.
