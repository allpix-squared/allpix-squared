...
InteractivePropagationModule::propagate_together(Event* event,
    ...
    std::function<Eigen::Vector3d(double, const Eigen::Vector3d&, allpix::CarrierType type)> carrier_velocity_noB =
        ...
        Eigen::Vector3d efield(raw_field.x(), raw_field.y(), raw_field.z()); //TODO: modify to include dynamic field too
    ...
    // Continue time propagation until the no more objects remain in propagating charge vector 
    Eigen::Vector3d last_position(propagating_charges[0].getLocalPosition());
    ROOT::Math::XYZVector efield{}, last_efield{};
    size_t next_idx = 0;
    while(propagating_charges.size() > 0) { //TODO: set up break if integration time is reached and trapped charges remain
        //TODO: Change loop explicitly loop from the minimum time of all charges and increase the time step each time. 
            //IDEA: Can use a minimum time variable that's increased each iteration

        // Loop over all charges remaining in the detector
        for (int i = 0; i < propagating_charges.size(); i++){

            auto charge = propagating_charges[i];
            auto position = Eigen::Vector3d(charge.getLocalPosition());
            ...
            if(state == CarrierState::TRAPPED){ // TODO: use state to affect what is performed in each iteration
                auto detrap_time = detrapping_(type, uniform_distribution(event->getRandomEngine()), std::sqrt(efield.Mag2()));
                if((charge.getLocalTime() + detrap_time) < integration_time_) {
                    ...
                    runge_kutta.advanceTime(detrap_time); // TODO: add check to change its state to MOTION after this time has passed.
            ...
            // Otherwise, the state is MOTION and we continue with the propagation
            ...
            auto step = runge_kutta.step(); // TODO: Check to make sure it is a MOTION state before advancing
            ...
            // TODO: Update all of the fields in propagating charges vector elements
            ...
            LOG(TRACE) << "Moving carriers below pixel " << Pixel::Index(xpixel, ypixel) << " from "
                    ...
                    << Units::display(charge.getLocalTime(), "ns");
            ...
            for(const auto& pixel_index : neighbors) {
                ...
                try {
                    pixel_map_iterator.first->second.addCharge(induced, charge.getLocalTime());
                } catch(const PulseBadAllocException& e) {
                    LOG(ERROR) << e.what() << std::endl
                            << "Ignoring pulse contribution at time "
                            << Units::display(charge.getLocalTime(), {"ms", "us", "ns"});
                }
                ...
                if(output_plots_) {
                    ...
                    induced_charge_histo_->Fill(charge.getLocalTime(), induced);
                    induced_charge_vs_depth_histo_->Fill(charge.getLocalTime(), position.z(), induced);
                    induced_charge_map_->Fill(inPixel_um_x, inPixel_um_y, induced);
                    if(type == CarrierType::ELECTRON) {
                        induced_charge_e_histo_->Fill(charge.getLocalTime(), induced);
                        induced_charge_e_vs_depth_histo_->Fill(
                            charge.getLocalTime(), position.z(), induced);
                        induced_charge_e_map_->Fill(inPixel_um_x, inPixel_um_y, induced);
                    } else {
                        induced_charge_h_histo_->Fill(charge.getLocalTime(), induced);
                    ...
                    if(!multiplication_.is<NoImpactIonization>()) {
                        induced_charge_primary_histo_->Fill(charge.getLocalTime(), induced_primary);
                        induced_charge_secondary_histo_->Fill(charge.getLocalTime(), induced_secondary);
                        if(type == CarrierType::ELECTRON) {
                            induced_charge_primary_e_histo_->Fill(charge.getLocalTime(), induced_primary);
                            induced_charge_secondary_e_histo_->Fill(charge.getLocalTime(),
                                                                    induced_secondary);
                        } else {
                            induced_charge_primary_h_histo_->Fill(charge.getLocalTime(), induced_primary);
                            induced_charge_secondary_h_histo_->Fill(charge.getLocalTime(),
                                                                    induced_secondary);
                        }
                    }
                }
        ...
        // TODO: Add a loop that checks if charges are finished propagating -> remove from vector, create and add propagatED charge object

    }

    // Perform the final output plots and 
