...
void ElectricFieldReaderModule::initialize() {
    ...
    if(field_model == ElectricField::MESH) {
        ...
        LOG(DEBUG) << "Electric field maps to " << enum_name(field_mapping);
...
void ElectricFieldReaderModule::create_output_plots() {
    ...
    // Determine the coordinate to use for projection and the axis titles for histograms
    double x = 0;
    double y = 0;
    double z = 0;
