...
* `mobility_model`: Charge carrier mobility model to be used for the propagation. Defaults to `jacoboni`, a list of available models can be found in the documentation. If the `masetti` or `masetti_canali` is used, the `n_dopant` parameter can be used to set the n-dopant to either phosphorous (default) or arsenic.
