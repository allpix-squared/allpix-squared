...
public:
    ...
    */
    explicit Tetrahedron(std::vector<Point> vertices_tetrahedron) : vertices(std::move(vertices_tetrahedron)) {}

    /**
    * @brief Explicit constructor with the mesh Points
    * @param p{i} Points from the mesh
    */
    Tetrahedron(Point v1, Point v2, Point v3, Point v4) : vertices{v1, v2, v3, v4} {}
    ...
    */
    Tetrahedron(Point v1, Point f1, Point v2, Point f2, Point v3, Point f3, Point v4, Point f4)
    ...
    * the mesh node
    */
    Tetrahedron(std::vector<Point> vertices_tetrahedron, std::vector<Point> efield_vertices_tetrahedron)
        : vertices(std::move(vertices_tetrahedron)), e_field(std::move(efield_vertices_tetrahedron)) {}

    /**
    * @brief Constructor using a vector to store the index of the noded and a std::vector<Points> for the mesh and electric
    * field Points
    * @param vertices_tetrahedron std::vector<Points> containing 4 mesh node Points
    * @param efield_vertices_tetrahedron std::vector<Points> containing 4 Points with the component of the electric field at
    * the mesh node
