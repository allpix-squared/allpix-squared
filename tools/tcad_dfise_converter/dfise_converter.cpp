...
Point barycentric_interpolation(Point query_point, std::vector<Point> tetra_vertices,
		                std::vector<Point> tetra_vertices_field, double tetra_volume){
...
//*/
    ...
    if(tetra_volume > 0) volume_signal = true;
    if(tetra_volume < 0) volume_signal = false;
    
    // Jacobi Matrix for volume calculation
    matrix_sub1 << 1, 1, 1, 1,
                   query_point.x, tetra_vertices[1].x, tetra_vertices[2].x, tetra_vertices[3].x, 
                   query_point.y, tetra_vertices[1].y, tetra_vertices[2].y, tetra_vertices[3].y, 
                   query_point.z, tetra_vertices[1].z, tetra_vertices[2].z, tetra_vertices[3].z; 
    tetra_subvol_1 = (matrix_sub1.determinant())/6;
    if(tetra_subvol_1 > 0) sub_1_signal = true;
    if(tetra_subvol_1 < 0) sub_1_signal = false;
    if(tetra_subvol_1 == 0) sub_1_signal = volume_signal;

    matrix_sub2 << 1, 1, 1, 1,
                   tetra_vertices[0].x, query_point.x, tetra_vertices[2].x, tetra_vertices[3].x, 
                   tetra_vertices[0].y, query_point.y, tetra_vertices[2].y, tetra_vertices[3].y, 
                   tetra_vertices[0].z, query_point.z, tetra_vertices[2].z, tetra_vertices[3].z; 
    tetra_subvol_2 = (matrix_sub2.determinant())/6;
    if(tetra_subvol_2 > 0) sub_2_signal = true;
    if(tetra_subvol_2 < 0) sub_2_signal = false;
    if(tetra_subvol_2 == 0) sub_2_signal = volume_signal;

    matrix_sub3 << 1, 1, 1, 1,
                   tetra_vertices[0].x, tetra_vertices[1].x, query_point.x, tetra_vertices[3].x, 
                   tetra_vertices[0].y, tetra_vertices[1].y, query_point.y, tetra_vertices[3].y, 
                   tetra_vertices[0].z, tetra_vertices[1].z, query_point.z, tetra_vertices[3].z; 
    tetra_subvol_3 = (matrix_sub3.determinant())/6;
    if(tetra_subvol_3 > 0) sub_3_signal = true;
    if(tetra_subvol_3 < 0) sub_3_signal = false;
    if(tetra_subvol_3 == 0) sub_3_signal = volume_signal;

    matrix_sub1 << 1, 1, 1, 1,
                   tetra_vertices[0].x, tetra_vertices[1].x, tetra_vertices[2].x, query_point.x, 
                   tetra_vertices[0].y, tetra_vertices[1].y, tetra_vertices[2].y, query_point.y, 
                   tetra_vertices[0].z, tetra_vertices[1].z, tetra_vertices[2].z, query_point.z; 
    tetra_subvol_4 = (matrix_sub1.determinant())/6;
    if(tetra_subvol_4 > 0) sub_4_signal = true;
    if(tetra_subvol_4 < 0) sub_4_signal = false;
    if(tetra_subvol_4 == 0) sub_4_signal = volume_signal;

    // Electric field interpolation
    efield_int.x = (tetra_subvol_1*tetra_vertices_field[0].x + tetra_subvol_2*tetra_vertices_field[1].x +
	            tetra_subvol_3*tetra_vertices_field[2].x + tetra_subvol_4*tetra_vertices_field[3].x)/tetra_volume;
    efield_int.y = (tetra_subvol_1*tetra_vertices_field[0].y + tetra_subvol_2*tetra_vertices_field[1].y +
	            tetra_subvol_3*tetra_vertices_field[2].y + tetra_subvol_4*tetra_vertices_field[3].y)/tetra_volume;
    efield_int.z = (tetra_subvol_1*tetra_vertices_field[0].z + tetra_subvol_2*tetra_vertices_field[1].z +
	            tetra_subvol_3*tetra_vertices_field[2].z + tetra_subvol_4*tetra_vertices_field[3].z)/tetra_volume;

    /* DEBUGGING prints (deactivate removing first slash)
    std::cout << " ===> Interpolating point q(" << query_point.x << "," << query_point.y << "," << query_point.z << ")" << std::endl;
    ...
    for (int i=0; i< tetra_vertices.size(); i++){
	    std::cout << "Vertex index:	" << vertex_index[i] <<
		    ...
		    "	Electric field:	(" << 
		    tetra_vertices_field[i].x << ", " <<  tetra_vertices_field[i].y << ", " << tetra_vertices_field[i].z << ")." << std::endl; 
    }
    ...
    std::cout << "Volume difference:	 " << tetra_volume - (tetra_subvol_1+tetra_subvol_2+tetra_subvol_3+tetra_subvol_4) << std::endl;
    std::cout << " ===> Electric field:	" << efield_int.x << " x, " << efield_int.y << " y, " << efield_int.z << " z" << std::endl << std::endl;
    //*/

    // Check if query point is outside tetrahedron
    if(sub_1_signal != volume_signal || sub_2_signal != volume_signal || sub_3_signal != volume_signal || sub_4_signal != volume_signal){
      throw std::exception();
    }
    ...
    for(double x = minx+(maxx-minx)/(2.0*xdiv); x <= maxx; x += (maxx-minx)/xdiv) {
      for(double y = miny+(maxy-miny)/(2.0*ydiv); y <= maxy; y += (maxy-miny)/ydiv) {
        for(double z = minz+(maxz-minz)/(2.0*zdiv); z <= maxz; z += (maxz-minz)/zdiv) {
          ...
          unsigned int ball=0;
          while(radius<5.5f){
            // Calling octree neighbours search function and sorting the results list with the closest neighbours first
              ...
              for(i2=1; i2<results.size(); i2++){
                ...
                for(i3=2; i3<results.size(); i3++){
                  ...
                  for(i4=3; i4<results.size(); i4++){
                    ...
                    matrix << 1, 		     1, 	            1,			   1,
                              points[results[i1]].x, points[results[i2]].x, points[results[i3]].x, points[results[i4]].x, 
                              points[results[i1]].y, points[results[i2]].y, points[results[i3]].y, points[results[i4]].y, 
                              points[results[i1]].z, points[results[i2]].z, points[results[i3]].z, points[results[i4]].z; 
                    ...
                    if(std::abs(volume) > 0.00001){
                      ...
                      try {
                        ...
                        //e = barycentric_interpolation(q, tetra_vertices, tetra_vertices_field, vertex_index, vertex_distance, volume, radius, results.size());
                      }
                      catch (std::exception) {
                  ...
                  if(tetra_vertices.size()==4) break;
                }                                               // end for 3
    ...
    for(int i=0; i<xdiv; ++i) {
        for(int j=0; j<ydiv; ++j) {
            for(int k=0; k<zdiv; ++k) {
                ...
                init_file << i+1 << " " << j+1 << " " << k+1 << " " << point.x << " " << point.y << " " << point.z << std::endl;
            }
        }
    }
    init_file.close();
