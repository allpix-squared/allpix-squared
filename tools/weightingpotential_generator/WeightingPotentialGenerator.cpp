...
int main(int argc, char** argv) {
    ...
    if(print_help) {
        ...
        std::cout << "\t --binning <int vector>  3D vector with the number of bins in each coordinate x, y, z" << std::endl;
        std::cout << "\t --matrix  <int vector>  2D vector with size of the pixel array in x and y the potential should be "
                     "calculated for"
                  << std::endl;
        std::cout << "\t --output  <file name>   Name of the file the potential should be stored in" << std::endl;
        std::cout << "\t --init                  Switch to enable writing the potential in the INIT format instead of APF"
                  << std::endl;
